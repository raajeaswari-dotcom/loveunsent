import { NextRequest } from 'next/server';
import connectDB from '@/lib/db';
import { Handwriting } from '@/models/Handwriting';
import { verifyToken } from '@/lib/auth';
import { successResponse, errorResponse } from '@/utils/apiResponse';
import { z } from 'zod';

const handwritingSchema = z.object({
    name: z.string(),
    imageUrl: z.string(),
    priceExtra: z.number(),
    isCursive: z.boolean().optional()
});

export async function POST(req: NextRequest) {
    try {
        await connectDB();
        const token = req.cookies.get('token')?.value;
        if (!token) return errorResponse('Unauthorized', 401);
        const decoded: any = verifyToken(token);
        if (!decoded || (decoded.role !== 'admin' && decoded.role !== 'super_admin')) return errorResponse('Forbidden', 403);

        const body = await req.json();
        const result = handwritingSchema.safeParse(body);
        if (!result.success) return errorResponse('Validation Error', 400, result.error.format());

        const style = await Handwriting.create(result.data);
        return successResponse({ style }, 201);
    } catch (error: any) {
        return errorResponse(error.message, 500);
    }
}

export async function GET(req: NextRequest) {
    try {
        await connectDB();
        const styles = await Handwriting.find({ isActive: true });
        return successResponse({ styles });
    } catch (error: any) {
        return errorResponse(error.message, 500);
    }
}
